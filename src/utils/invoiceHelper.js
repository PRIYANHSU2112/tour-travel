...
class InvoiceHelper {
  static getInvoiceHTML(booking) {
    ...
    const statusColor = booking.paymentStatus === 'Paid' ? '#27AE60' : 
                       booking.paymentStatus === 'Partial' ? '#F39C12' : 
                       booking.paymentStatus === 'Failed' ? '#E74C3C' : '#3498DB';
    ...
    const statusBadgeHTML = `<div class="status-badge" style="background: ${statusColor};">${booking.paymentStatus || 'Pending'}</div>`;
    ...
    const replacements = {
      ...
      '{{packageCostPerPerson}}': formatCurrency(booking.packageCostPerPerson || 0),
    ...
    const hasTravelerDetails = booking.travelerDetails && booking.travelerDetails.length > 0;
    html = html.replace(/\{\{#if travelerDetails\}\}([\s\S]*?)\{\{\/if\}\}/g, 
      hasTravelerDetails ? '$1' : '');

    html = html.replace(/\{\{#if paymentMethod\}\}([\s\S]*?)\{\{\/if\}\}/g, 
      booking.paymentMethod ? '$1' : '');

    html = html.replace(/\{\{#if transactionId\}\}([\s\S]*?)\{\{\/if\}\}/g, 
      booking.transactionId ? '$1' : '');

    const hasDiscount = booking.discountAmount && booking.discountAmount > 0;
    html = html.replace(/\{\{#if hasDiscount\}\}([\s\S]*?)\{\{\/if\}\}/g, 
      hasDiscount ? '$1' : '');

    html = html.replace(/\{\{#if addOns\}\}([\s\S]*?)\{\{\/if\}\}/g, 
      hasAddOns ? '$1' : '');
