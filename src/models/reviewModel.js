...
reviewSchema.pre('save', async function (next) {
  ...
  if ( (this.isModified('location') || this.isModified('services') || this.isModified('price')) && this.placeId) {
    await updatePlaceAverages(this.placeId)
  }
  if ( (this.isModified('location') || this.isModified('services') || this.isModified('price')) && this.packageId) {
...
async function updatePlaceAverages(placeId) {
  try {
    ...
    if (result.length > 0) {
      const { avgRating, totalReviews, avgLocation, avgPrice, avgServices } = result[0];
