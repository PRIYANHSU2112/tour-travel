...
class PackageController {
  ...
  async getPackages(filter = {}, options = {}) {
    ...
    if (Object.prototype.hasOwnProperty.call(normalizedFilter, "duration")) {
      ...
      if (normalizedFilter.duration && normalizedFilter.duration.trim() && !isNaN(Number(normalizedFilter.duration.trim()))) {
    ...
    if (Object.prototype.hasOwnProperty.call(normalizedFilter, "minDuration") && Object.prototype.hasOwnProperty.call(normalizedFilter, "maxDuration")) {
      if (normalizedFilter.minDuration && normalizedFilter.maxDuration && normalizedFilter.minDuration.trim() && normalizedFilter.maxDuration.trim() && !isNaN(Number(normalizedFilter.minDuration.trim())) && !isNaN(Number(normalizedFilter.maxDuration.trim()))) {
        ...
        normalizedFilter.durationDays = { $gte: minDuration, $lt: maxDuration + 1 }
      }
      delete normalizedFilter.minDuration;
      delete normalizedFilter.maxDuration;
    ...
    if (Object.prototype.hasOwnProperty.call(normalizedFilter, "rating")) {
      ...
      if (normalizedFilter.rating && normalizedFilter.rating.trim() && !isNaN(Number(normalizedFilter.rating.trim()))) {

        const value = normalizedFilter.rating.trim();
        if (value) {
          const rating = Math.min(Math.max(Number(value), 1), 5)
          normalizedFilter['ratings.averageRating'] = { $gte: rating, $lt: rating + 1 }
    ...
    if (Object.prototype.hasOwnProperty.call(normalizedFilter, "search")) {
      ...
      if (value && value.trim()) {
        ...
        normalizedFilter.$or = [{ packageName: regex }, { tagline: regex }, { slug: regex }];
      }
  ...
  async getPackageById(id) {
    return this.model.findById(id).populate("cityIds").populate("itinerary.placeIds");
  }
