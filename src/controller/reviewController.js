...
const DEFAULT_TESTIMONIALS_PAGE_SIZE = parseInt(process.env.DEFAULT_TESTIMONIALS_PAGE_SIZE || "5", 5);
...
class ReviewController {
    ...
    async addReview(payload) {
        ...
        const { userId, review, placeId, packageId ,guideId,tourId,bookingId} = payload
    ...
    async getReview(options = {}, filters = {}) {
        ...
        if (filters.rating && filters.rating.trim() && !isNaN(Number(filters.rating.trim()))) {
        ...
        console.log(normalizedFilter)
        const query = this.model.find(normalizedFilter).skip((currentPage - 1) * pageSize).limit(pageSize).populate("placeId userId packageId").lean()
...
async getDistinctTopReviews(limit) {
        ...
        const parsedPage = !Number.isNaN(parsedLimit) && parsedLimit > 0 ? parsedLimit : DEFAULT_TESTIMONIALS_PAGE_SIZE;
