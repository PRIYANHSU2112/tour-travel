...
router.get("/" , async (req , res)=>{
   try {
    ...
    const wishlist = await reviewController.getReview( {page,limit , sort},filters);
    if (!wishlist) {
      return res.status(404).json({ success: false, message: "User or Place not found" });
    }
    res.status(200).json({
   ...
   try {
         ...
         const { userId, placeId } = req.params;
          ...
          if(req.user.role !=='Admin' && realUserId !== userId){
            return res.status(403).json({ success: false, message: "Forbidden: You don't have permission to delete this review." });
          }
