...
router.patch("/:id/disable",protect, async (req, res) => {
  try {
    ...
    const booking = await bookingController.setBookingDisabled(req.params.id, { isDisabled });
    if (!booking) {
      return res.status(404).json({ success: false, message: "Booking not found" });
    }
    ...
    res.status(200).json({
      success: true,
      message: booking.isDisabled ? "Booking disabled successfully" : "Booking enabled successfully",
      data: booking,
    });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
