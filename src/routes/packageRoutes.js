...
const { packageModel, packageTypes, mealOptions, transportOptions } = require("../models/packageModel");
...
router.post("/",protect, async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ success: true, message: "Package created successfully", data: travelPackage });
  } catch (error) {
...
router.get("/:id", async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "Package fetched successfully", data: travelPackage });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
});
...
router.put("/:id",protect, async (req, res) => {
  try {
    const travelPackage = await packageController.updatePackage(req.params.id, req.body);
    if (!travelPackage) {
      return res.status(404).json({ success: false, message: "Package not found" });
    }
    res
      .status(200)
      .json({ success: true, message: "Package updated successfully", data: travelPackage });
  } catch (error) {
    res.status(400).json({ success: false, message: error.message });
  }
});
...
router.post("/:id/duplicate",protect, async (req, res) => {
  try {
    ...
    res.status(201).json({ success: true, message: "Package duplicated successfully", data: duplicate });
  } catch (error) {
    res.status(400).json({ success: false, message: error.message });
  }
});
...
router.patch("/:id/disable",protect, async (req, res) => {
  try {
    ...
    const travelPackage = await packageController.setPackageDisabled(req.params.id, { isDisabled });
    if (!travelPackage) {
      return res.status(404).json({ success: false, message: "Package not found" });
    }
...
router.get("/meta/enums", (req, res) => {
  res
    .status(200)
    .json({ success: true, data: { packageTypes, mealOptions, transportOptions } });
});
