...
const { placeModel, accessibilityOptions, crowdLevels } = require("../models/placeModel");
...
router.get("/:id", async (req, res) => {
  try {
    ...
    res.status(200).json({ success: true, message: "Place fetched successfully", data: place });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
});
...
router.patch("/:id/disable", protect, async (req, res) => {
  try {
    ...
    const place = await placeController.setPlaceDisabled(req.params.id, { isDisabled });
    if (!place) {
      return res.status(404).json({ success: false, message: "Place not found" });
    }
    ...
    res.status(200).json({
      success: true,
      message: place.isDisabled ? "Place disabled successfully" : "Place enabled successfully",
    });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
...
router.get("/filter/data", async (req, res) => {
  try {
    ...
    // console.log(tempo)
    //  const query3=packageModel.distinct("durationDays")

    const query5 = packageModel.find().sort({ basePricePerPerson: 1 }).limit(1).select("basePricePerPerson")
    const query6 = packageModel.find().sort({ basePricePerPerson: -1 }).limit(1).select("basePricePerPerson")
    const [countryData, cityData, minPrice, maxPrice] = await Promise.all([query1, query2, query5, query6]);
    const priceFilter = {
